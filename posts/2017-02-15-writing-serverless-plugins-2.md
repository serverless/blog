...
description: "Learn how to write a Serverless plugin in this 2-part guest post from Anna Doubkova. Part 1 covered the basics of creating a plugin. Now Part 2 explores implementation and different approaches to writing Serverless plugins."
date: 2017-02-15
...
Plugins extend functionality of the framework to tailor it for your use-case. The framework is very flexible and allows you to take different strategies to implementing your logic. The main ways are:
...
## Copy Data Plugin
We can start by defining the command in a new class as shown in [Part 1 of this series](https://serverless.com/blog/writing-serverless-plugins/). I imagine the command would have two steps or lifecycle events - one for downloading the data and one for uploading them. It could look something like this:
...
When the command is defined, we attach lifecycle functions by defining them as hooks. Again, one function will handle downloading the data and the other one will upload them. Notice that we're binding `serverless` and `options` to these functions so that they can access their data.
...
However, I personally prefer more functional approach. That's why we'll carry on in this tutorial with the two functions completely separate from the class itself - as per `Version 1`.

Having defined the command, it's time to jump to the implementation.

## Downloading Data
First, we want to download the data from a production database and save them somewhere so that we can upload them in the second step. I decided in this case to save the downloaded data to `serverless.variables` so that it can be easily accessed.
...
As downloading is likely an asynchronous event, our function needs to return a promise. This way, the Serverless Framework will know to wait for this step to finish before starting the following one.

Another handy thing to notice here is `cli.log()` function on the serverless object. It provides us with a uniform message logs to the console.

_Note:_ In your implementation, you can easily swap DynamoDB for another database (or even provider!) entirely - however for now, I'd like to stick to AWS as the most popular provider.

## Uploading Data
In the second step of the implemention, we simply need to take data from `serverless.variables.copyData` and upload them to the test/dev database. As far as I'm aware, we can only do that by uploading the data one by one:
...
## Defining Stage
Our plugin is so far really useful only if I have a `users` table deployed to `production` and `dev`. I might as well want to use it for `customers`, moving data from `test` to `dev` for debugging and testing purposes.

We have already used region to configure `aws-sdk`. For the others, we need to first consider what we might have defined in `serverless.yml`. One of the typical set-ups would be to have a default stage that can be optionally replaced by passing `--stage` flag to `sls`.
...
Now in the upload function, we update table name same way we did in upload; this time however, getting target from the options.
...
Now the plugin can get data from `users` table in any stage and upload them to another one by running `sls copy-data -s production -t dev`.

## Which Table?
To get the right table, you can again specify an option and swap `users` for `options.tableName`. However, there's another way to do it, that illustrates abilities of the Serverless Framework.
...
Although the format of the table name is still `name-stage`, it won't work with our plugin because it expects a `user` table. Luckily, the format of the data can be defined in any way, so we only need to change the way we're getting the table name.

We add a new option `--resource` that will point to the Resource name. Then we get the right table name from its Resource definition, and can replace original stage with the target one:
...
## Plugin Done
Well done! You have just finished a Serverless Framework plugin that is not only solving a real-world issue, but also can be easily packaged, published to npm and shared with others!
