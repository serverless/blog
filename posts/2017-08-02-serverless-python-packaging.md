...
A second problem is with the import path. You can install your dependencies directly into your top-level directory, but that clutters up my workspace. If you install them into a subdirectory like `deps/` or `vendored/`, you have to mess with your `sys.path` at the beginning of your function.

In this post, I'll show you a better way using the magical [`serverless-python-requirements`](https://github.com/UnitedIncome/serverless-python-requirements) plugin for the Serverless Framework.

# Initial Setup

First, let's get our environment ready. If you have Node and NPM installed, install the Serverless Framework globally with:

```bash
$ npm install -g serverless
```

You'll also need to configure your environment with AWS credentials. If you need any assistance with installing the Framework or getting AWS credentials, check out the Prerequisites portion of our [Quick Start Guide](https://serverless.com/framework/docs/providers/aws/guide/quick-start/).
...
First, let's create a service from a template. I'm going to use Python 3, but this works with Python 2 as well.

```bash
$ serverless create --template aws-python3 --name numpy-test --path numpy-test
```

This will create a Serverless Python 3 template project at the given path (`numpy-test/`) with a service name of `numpy-test`.

Next, change into that directory and create a virtual environment for developing locally. Read [here](http://python-guide-pt-br.readthedocs.io/en/latest/dev/virtualenvs/) about how and why to use virtual environments with Python.
...
Now, let's set up the function we want to deploy. Edit the contents of `handler.py` so that it contains the following:
...
Uh ohh, we haven't installed `numpy` in our virtual environment yet. Let's do that now, and save the package versions of our environment to a `requirements.txt` file:
...
Now if we run our command locally, we'll see the output we want:
...
Now that we have our function working locally, let's get it deployed to Lambda. First, edit the `serverless.yml` file to look like the following:
...
Our last step before deploying is to add the `serverless-python-requirements` plugin. First, create a package.json file for saving your node dependencies. Accept the defaults, then install the plugin:
...
Then, let's configure our `serverless.yml` file to use the plugin. Add the following lines in your `serverless.yml`:
...
Now let's invoke our `numpy` function and read the logs:
...
Magic! You've got NumPy in your Lambda!

Many thanks to the United Income team and [@dschep](https://github.com/dschep) in particular for creating the `serverless-python-requirements` package. Be sure to check out the repo for additional functionality, including automatic compression of libraries before deploying, which can be a huge help with the larger numerical libraries in Python.