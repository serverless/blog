...
AWS gives you some powerful building blocks for making amazing applications. In this walkthrough, we're going to create a multi-region, multi-master, geo-routed application in about 30 minutes. That's pretty mind-blowing. We'll have compute located on two different continents writing to databases in their own region, and the written data will be replicated to the database in the other region. Users will be directed to the closest region according to their location.

Prefer the video walkthrough? Check out the recorded video from the webinar with our own Rupak Ganguly plus the prolific Alex Casalboni from Cloud Academy. TODO
...
I'm going to write this in Python because I love Python. However, you can do it in any language you want.
...
- a `set_key` function to allow users to store the value for a key; and
...
Then, add the code for your `setKey` function in `handlers/set_key.py`:
...
Let's deploy it! I'm going to use the `us-west-2` region:
...
If your deploy was successful, it will show your base url in the `endpoints` block of your Service Information. Let's test our endpoints. Export your url into a variable called BASE_URL, then run a curl command to set a key:
...
We set a key! Let's retrieve it:
...
Great! We retrieved our key. Note that it says it was last written in the `us-west-2` region and that we're currently reading from the `us-west-2` region.

## Adding a custom domain

Now, let's add a custom domain to our endpoint. This means we won't have a funky endpoint like `6rbm4350zj.execute-api.us-west-2.amazonaws.com`, plus it will help with our geo-routing later. To follow along here, *you will need to own a domain registered in Route53*.

We're going to use the [serverless-domain-manager](https://github.com/amplify-education/serverless-domain-manager) plugin to simplify this. We already have a blog post on [setting up your custom domain with Serverless & API Gateway](https://serverless.com/blog/serverless-api-gateway-domain/) if you want more details on it. This usage will be a little bit different since we're using a new feature to [set up regional domains](https://github.com/amplify-education/serverless-domain-manager/pull/49).

My base domain is `serverlessteam.com`. I want my app to generally be accessible at `keyvalues.serverlessteam.com`.

First, let's create an SSL certificate for our domain using Amazon Certificate Manager (ACM). Navigate to Amazon Certificate Manager _in the region you want to deploy your endpoint_. Click "Request a certificate" and enter your domain. I'll create a certificate at `keyvalue.serverlessteam.com`. 
...
Then configure your `serverless.yml` by adding the plugin to `plugins` and adding config in the `custom` block:
...
This will help us provision a custom domain in API Gateway. Note that we're provisioning a `regional` endpointType, and the domain name will use the region. Finally, we won't have it create our Route53 record as we'll create that separately. 

Then, create your domain:
...
First, set a key in your US region:
...
The key doesn't show up! This is because our two regions are completely separate from each other. Each has their own Lambda compute and their own DynamoDB data store. While these separate regions allow for fast queries around the globe, it also results in a segmented experience for users. We want the _data_ to be shared, so a user gets a fast, consistent experience at any point on earth.

## DynamoDB Global Tables

Enter DynamoDB Global Tables. This feature, announced at AWS re:Invent 2017, allows you to specify DynamoDB tables in separate regions to act as a single table. Writes in one region will be asynchronously replicated to the other regions. This allows for some powerful applications without writing custom syncing logic.
...
If you have Items in your table, use the script below to clean them out. Note that you'll need to run it for each region that has a table with Items in it:
...
Let's test it out. First, save a key in the US region:
...
It worked! We can read the key in a different region. Note that the response from the `getKey` method includes the region in which the key was written (`us-west-2`) as well as the region from which we're currently reading (`eu-central-1`).
